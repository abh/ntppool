...
my %language_chars = (
    ...
    'es' => {
        ...
        common_words =>
          ['información', 'configuración', 'también', 'después', 'así', 'más'],
    },
    'fr' => {
        name           => 'French',
        expected_chars => [
            'à', 'â', 'ä', 'ç', 'è', 'é', 'ê', 'ë', 'î', 'ï', 'ô', 'ù', 'û', 'ü',
            'ÿ', 'À', 'Â', 'Ç', 'È', 'É', 'Ê', 'Ë', 'Î', 'Ï', 'Ô', 'Ù', 'Û', 'Ü'
        ],
    ...
    'pt' => {
        name           => 'Portuguese',
        expected_chars => [
            'á', 'â', 'ã', 'à', 'ç', 'é', 'ê', 'í', 'ó', 'ô', 'õ', 'ú', 'ü', 'Á',
            'Â', 'Ã', 'À', 'Ç', 'É', 'Ê', 'Í', 'Ó', 'Ô', 'Õ', 'Ú'
        ],
        common_words =>
          ['configuração', 'não', 'informações', 'versão', 'também', 'operação'],
    },
    'sv' => {
        name           => 'Swedish',
    ...
    'fi' => {
        ...
        common_words   =>
          ['käyttää', 'määrittää', 'työskentelyn', 'sähköposti', 'yhteisö'],
    },
    'nb' => {
        ...
        if ($line =~ /\x{FFFD}/) {
            push @issues,
              {   type    => 'replacement_character',
                  line    => $line_num,
                  message =>
                  "Replacement character (�) found - indicates encoding corruption",
                  severity => 'high'
              };
        }
        ...
        for my $entity (keys %suspicious_entities) {
            if ($line =~ /\Q$entity\E/) {
                ...
                if ($language_chars{$lang}
                    && grep { $_ eq $native_char }
                    @{$language_chars{$lang}{expected_chars}})
                {
                    ...
                    push @issues,
                      {   type    => 'should_be_native',
                          ...
                          line    => $line_num,
                          message =>
                          "HTML entity '$entity' should be native UTF-8 '$native_char' for $lang",
                          severity => 'medium'
                      };
                }
            }
...
sub check_language_characters {
    ...
    if ($expected_ratio < 0.1 && @expected_chars > 5) {
        push @issues,
          {   type     => 'missing_language_chars',
              ...
              message  =>
              "Very few expected $lang_info->{name} characters found ($expected_ratio ratio)",
              severity => 'medium'
          };
    }
    ...
    for my $word (@common_words) {
        ...
        if (   $ascii_word ne $word
            && $content =~ /\Q$ascii_word\E/
            && $content !~ /\Q$word\E/)
        {
            push @issues,
        ...
        if ($line =~ /[a-zA-Z]\x{0300}-\x{036F}/) {
            push @issues,
              {   type    => 'possible_nfd',
                  line    => $line_num,
                  message =>
                  "Possible NFD normalization (separate base + combining characters)",
                  severity => 'low'
              };
        }
    }
...
sub analyze_language_encoding {
    ...
    $report{files_with_issues} =
      scalar(keys %{$report{html_files} || {}}) + (exists $report{po_issues} ? 1 : 0);
...
sub format_issues {
    ...
    for my $severity ('high', 'medium', 'low') {
        ...
        my $color =
            $severity eq 'high'   ? $colors{red}
          : $severity eq 'medium' ? $colors{yellow}
          :                         $colors{cyan};
...
sub format_language_report {
    ...
    $output
      .= "Files checked: $report->{files_checked}, Issues found: $colors{red}$report->{total_issues}$colors{reset}\n";
...
sub generate_summary {
    ...
    $output
      .= "Languages with encoding issues: $colors{red}$langs_with_issues$colors{reset}\n";
    $output
      .= "Languages with clean encoding: $colors{green}$langs_clean$colors{reset}\n\n";

    $output .= "Issue breakdown by severity:\n";
    $output
      .= "  - $colors{red}High:$colors{reset} $high_severity (encoding errors, corruption)\n";
    $output
      .= "  - $colors{yellow}Medium:$colors{reset} $medium_severity (HTML entities, missing chars)\n";
    $output
      .= "  - $colors{cyan}Low:$colors{reset} $low_severity (normalization, counts)\n";

...
my @expected_hosting_providers = ('Equinix', 'Netactuate',);
        ...
        if (@missing_providers) {
            push @issues,
              {   type      => 'missing_providers',
                  ...
                  message   => "Missing current hosting providers: "
            ...
            if (grep { $_ eq $found_count } @{$ref->{outdated}}) {
                push @issues,
                  {   type     => 'outdated_server_count',
                      ...
                      message  =>
                      "Server count recommendation '$found_count' should be '$ref->{expected}'"
                  };
            }
        }
        ...
        if (@found_domains != @expected_domains) {
            push @issues,
              {   type     => 'pool_domains_mismatch',
                  ...
                  message  => "Pool domains mismatch: found "
                  . scalar(@found_domains)
                  . " expected "
                  . scalar(@expected_domains)
              };
        }
    }
    ...
    if (@en_code_blocks != @lang_code_blocks) {
        push @issues,
          {   type       => 'code_block_count_mismatch',
              ...
              . scalar(@lang_code_blocks)
              . " vs English "
              . scalar(@en_code_blocks)
          };
    }
    ...
    if ($content =~ /Control Panel/i && $content !~ /Win\+I|Settings/i) {
        push @issues,
          {   type    => 'outdated_windows_instructions',
              message =>
              "Windows instructions may be outdated (mentions Control Panel but not modern Settings app)"
          };
    }
...
sub format_language_report {
    ...
    $output
      .= "Total consistency issues: $colors{red}$report->{total_issues}$colors{reset}\n";
    ...
    if ($report->{hosting_issues}) {
        ...
        for my $issue (@{$report->{hosting_issues}}) {
            my $color =
              $issue->{type} eq 'outdated_provider' ? $colors{red} : $colors{yellow};
...
sub generate_summary {
    ...
    $output
      .= "Languages with consistency issues: $colors{red}$langs_with_issues$colors{reset}\n";
    ...
    $output
      .= "  - Content structure issues: $colors{yellow}$structure_issues$colors{reset}\n";
    $output
      .= "  - Technical reference issues: $colors{cyan}$technical_issues$colors{reset}\n";
...
my @consistent_langs =
  grep { $all_reports{$_}{total_issues} == 0 } sort keys %all_reports;
if (@consistent_langs) {
